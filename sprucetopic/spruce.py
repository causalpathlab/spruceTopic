...
from model import _cell_topic,_interaction_topic,_interaction_topic_ddb
import pandas as pd
import numpy as np
import torch
...
class Spruce:
    ...
    def run_interaction_topic(self,batch_size,epochs,layers1,layers2,latent_dims,input_dims1,input_dims2,device,f_loss):
        ...
        dl = _lr_augment.load_data(self.data.cell_topic_h, self.data.raw_l_data, self.data.raw_r_data, self.data.raw_lr_data, self.data.neighbour, batch_size,device)
    ...
    def run_interaction_topic_ddb(self,batch_size,epochs,layers1,layers2,latent_dims,input_dims1,input_dims2,device,f_loss):
        ...
        logging.info('Input dimension - ligand is '+ str(input_dims1))
        logging.info('Input dimension - receptor is '+ str(input_dims2))
        model = _interaction_topic_ddb.LitETM(input_dims1,input_dims2,latent_dims,layers1,layers2,f_loss)
        logging.info(model)

        trainer = _interaction_topic_ddb.pl.Trainer(
        max_epochs=epochs,
        accelerator='gpu',
        plugins= _interaction_topic_ddb.DDPPlugin(find_unused_parameters=False),
        gradient_clip_val=0.5,
        progress_bar_refresh_rate=50,
        enable_checkpointing=False)
    ...
    def eval_interaction_topic_ddb(self,input_dims1,input_dims2,latent_dims,layers1,layers2):

        model = _interaction_topic_ddb.LitETM(input_dims1,input_dims2,latent_dims,layers1,layers2,'temp.txt')
        ...
        beta_smax = _interaction_topic_ddb.nn.LogSoftmax(dim=-1)
        alpha = _interaction_topic_ddb.torch.exp(beta_smax(alpha))
        beta = _interaction_topic_ddb.torch.exp(beta_smax(beta))
