...
def umap_plots_kmeans_label():
    ...
    print(df_umap.columns)
    colors =  ["orange", "#1CE6FF", "fuchsia", "#7A4900", "green","#006FA6" ,"limegreen", "red", "blue"]

    cp = sns.color_palette(colors,9)
    p = sns.scatterplot(data=df_umap, x='umap1', y='umap2', hue='celltype',s=1,palette=cp,legend=False)
    # plt.legend(title='Cell type',title_fontsize=18, fontsize=14,loc='center left', bbox_to_anchor=(1, 0.5))
    ...
    # celltype_ct = df_umap['celltype_ct']
    # umap1 = df_umap['umap1']
    # umap2 = df_umap['umap2']
    # marked = []
    # for i, ct in enumerate (celltype_ct):
    #     m = ct.split('/')[1]
    #     # if m not in marked:
    #     plt.annotate(m, (umap1[i]+0.1, umap2[i]+0.1) )
    #         # marked.append(m)
...
def latent_hist_plots_withcelltype():
    ...
    selected_topics = [0,1,2,4,6,7,9,12,14,17,19,20,21,22,23,24,26,27,28,30,31,32,33,34,37,38,39,43,45,46,48]
    dfs = dfs[dfs['cell_topic'].isin(selected_topics)]
    ctmap = {
        0:'Myeloid/0',1:'Myeloid/1',2:'Cancer/2',4:'B/4',6:'Myeloid/6',7:'CAF/7',9:'Cancer/9',12:'Epithelial/12',14:'Plasma/14',17:'Epithelial/17',19:'Cancer/19',20:'B/20',21:'Cancer/21',22:'CAF/22',23:'T/23',24:'Cancer/24',26:'PVL/26',27:'Cancer/27',28:'Endothelial/28',30:'T/30',31:'Cancer/31',32:'Cancer/32',33:'Cancer/33',34:'Endothelial/34',37:'T/37',38:'B/38',39:'T/39',43:'Plasma/43',45:'B/45',46:'Epithelial/46',48:'Cancer/48'}
    dfs['cell_topic'] = [ctmap[x] for x in dfs['cell_topic']]
    # dfs['cell_topic'] = dfs['cell_topic'].astype(str)
    # df_umap = pd.read_csv(spr.cell_topic.id +'1_d_celltopic_label.csv.gz')
    ...
    # print(df_umap_sum.cell_topic.unique())
    ...
    dfs = dfs.sort_values('cell_topic')

    colors =  ["orange", "#1CE6FF", "fuchsia", "#7A4900", "green","#006FA6" ,"limegreen", "red", "blue"]

    hue_order = ['B','CAF','Cancer','Endothelial','Epithelial','Myeloid','PVL','Plasma','T']

    ax=sns.histplot(data=dfs,x="cell_topic",
    hue="celltype",hue_order = hue_order,weights="ncount",multiple="stack",discrete=True,palette=sns.color_palette(colors, 9))
    ...
    ax=sns.histplot(data=dfs,x="cell_topic",
    hue="celltype",hue_order = hue_order,weights="celltype_val",multiple="stack",discrete=True,palette=sns.color_palette(colors, 9))
    ax.set_xlabel("Cell topic",fontsize=20)
    ax.set_ylabel("Cell type distribution",fontsize=20)
    plt.xticks(rotation=45)
    sns.move_legend(ax, "center left", bbox_to_anchor=(1, 0.5))
