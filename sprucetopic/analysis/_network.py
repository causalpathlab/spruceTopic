...
def lr_correlation_network_db(spr,df,interact_topics,zcutoff,corr_th,df_db):
	...
	for i,topic in enumerate(interact_topics):
		...
		## keep only lr_db 
		df_corr = df_corr[df_corr.index.isin(df_db['ligand_gene_symbol'].values)]
		selected_receptors = [ x for x in df_corr.columns if x in df_db['receptor_gene_symbol'].values]
		df_corr = df_corr[selected_receptors]
		
		df_corr = df_corr.reset_index().melt(id_vars=['index'])
		df_corr.columns = ['ligands','receptors','score']

		lr_pair = {}
		for x in df_db['lr_pair'].values:
			l = x.split('_')[0]
			r = x.split('_')[1]
			if l not in lr_pair.keys(): lr_pair[l] =[r]
			else:lr_pair[l] = lr_pair[l] + [r] 
		...
		for row in df_corr.iterrows():
			l = row[1]['ligands']
			r = row[1]['receptors']
			if l in lr_pair.keys():
				if r in lr_pair[l]: keep.append(1)
				else:keep.append(0)
			else:keep.append(0)

		df_corr['keep'] = keep
		df_corr = df_corr[df_corr['keep']==1]
		df_corr = df_corr.drop(columns='keep')
...
def lr_correlation_network_stringdb(spr,df,interact_topics,zcutoff,corr_th,df_db):
	...
	for i,topic in enumerate(interact_topics):
		...
		## keep only lr_db 
		df_corr = df_corr[df_corr.index.isin(df_db['ligands'].values)]
		selected_receptors = [ x for x in df_corr.columns if x in df_db['receptors'].values]
		df_corr = df_corr[selected_receptors]
