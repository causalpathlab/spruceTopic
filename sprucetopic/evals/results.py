...
def cell_interaction_network(args):
	...
	df['label'] = pd.merge(df,df_meta, on='cell',how='left')['celltype_major'].values
...
def cancer_centric_view_lr(args):
	...
	top_r_genes = []
	top_r_genes = generate_gene_vals(df_beta1,5,top_r_genes,'receptors')
	top_r_genes = pd.Series([x[3] for x in top_r_genes]).unique()

	top_l_genes = []
	top_l_genes = generate_gene_vals(df_beta2,5,top_l_genes,'ligands')
	...
	model_file = args_home+args.output+args.lr_model['out']+args.lr_model['mfile']
	df_l.to_csv(model_file+'_cv_ligands.tsv.gz',index=False,sep='\t',compression='gzip')
	df_r.to_csv(model_file+'_cv_receptors.tsv.gz',index=False,sep='\t',compression='gzip')
