...
i_clust$Clust_CD4 = factor(i_clust$Clust_CD4)
i_clust$Clust_CD8 = factor(i_clust$Clust_CD8)
rownames(i_clust)= i_clust$Gene
i_clust[order(i_clust$Gene),]
i_clust = i_clust[,c('Clust_CD4','Clust_CD8')]

newCols <- colorRampPalette(grDevices::rainbow(length(unique(i_clust$Clust_CD4))))
cd4c <- newCols(length(unique(i_clust$Clust_CD4)))
names(cd4c) <- unique(i_clust$Clust_CD4)
cd8c <- newCols(length(unique(i_clust$Clust_CD8)))
names(cd8c) <- unique(i_clust$Clust_CD8)
...
p1 <- pheatmap(df_topic_i,annotation_col = i_clust ,annotation_colors = my_colour,
...
weightmat_phmap_plot_ni <- function(args) {
topgenes_file = paste(args_home,args$output,args$nbr_model$out,args$nbr_model$mfile,"top_5_genes_topic.tsv",sep="")
df_tg = read.table(topgenes_file, sep = "\t", header=TRUE)
...
df_tg$Topic <- factor(df_tg$Topic,levels = c("k0", "k1", "k2", "k3", "k4","k5",
      "k6","k7","k8","k9","k10","k11","k12","k13","k14","k15"))
df_tg$Proportion = log(df_tg$Proportion)
df_topic_i = as.matrix(cast( df_tg[df_tg$GeneType=="non-immune",] , Topic~Gene) )
