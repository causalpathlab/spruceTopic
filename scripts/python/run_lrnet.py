...
def run_model(mode):
	...
	if mode == 'generate_tensor':
		...
		dl = litlrnet.load_data(args,nbr_size,batch_size)
		...
		model = litlrnet.LitETM(input_dims1,input_dims2,latent_dims,layers1,layers2)
		logging.info(model)

		trainer = litlrnet.pl.Trainer(
		max_epochs=epochs,
		accelerator='gpu',
		plugins=litlrnet.DDPPlugin(find_unused_parameters=False),
