...
from analysis import _topics,_umap_viz,_gsea
import logging
import pandas as pd
import spruce
import torch
...
if mode =='train':
	...
	sp.cell_topic.beta_mean = pd.read_csv(sp.model_id+'_cell_topic_beta.tsv.gz',sep='\t',compression='gzip')
	sp.cell_topic.beta_var = pd.read_csv(sp.model_id+'_cell_topic_beta_var.tsv.gz',sep='\t',compression='gzip')
	...
	# df_htest.loc[df_htest['pval']<0.01,['pval']] = 1.0
	df_htest = df_htest.sort_values('pval')
	df_htest.to_csv(sp.model_id+'_cmark_hypergeom_test.tsv.gz',compression='gzip')	
