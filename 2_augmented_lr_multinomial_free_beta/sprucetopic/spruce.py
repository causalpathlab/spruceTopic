...
class Spruce:
    ...
    def run_interaction_topic(self,batch_size,epochs,layers1,layers2,latent_dims,input_dims1,input_dims2,device,f_loss):
        ...
        model = _interaction_topic_dir.LitETM(input_dims1,input_dims2,latent_dims,layers1,layers2,f_loss)
        logging.info(model)

        trainer = _interaction_topic_dir.pl.Trainer(
        max_epochs=epochs,
        accelerator='gpu',
    ...
    def eval_interaction_topic(self,input_dims1,input_dims2,latent_dims,layers1,layers2):
        ...
        model = _interaction_topic_dir.LitETM(input_dims1,input_dims2,latent_dims,layers1,layers2,'temp.txt')
        ...
        for n,p in model.named_parameters():
            print(n)
            if n == 'etm.decoder.p_beta1':
                beta1=p
            elif n == 'etm.decoder.p_beta2':
                beta2=p
            elif n == 'etm.decoder.p_beta1_bias':
                beta1_bias=p
            elif n == 'etm.decoder.p_beta2_bias':
                beta2_bias=p
    ...
    def interactions_state_summary(self,input_dims1,input_dims2,latent_dims,layers1,layers2):
        ...
        model = _interaction_topic_dir.LitETM(input_dims1,input_dims2,latent_dims,layers1,layers2,'_txt_')
        model.load_state_dict(self.interaction_topic.model)
        model.eval()
